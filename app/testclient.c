...
int sendQueryViaTCP(unsigned char *buf, int query_len, unsigned char *dest) {
    ...
    if (connect(socket_desc, (struct sockaddr *)&dest_addr,
    ...
    if (recv(socket_desc, (char *)buf, BUF_SIZE, 0) <= 0) {
...
dns_resource_record_t *getHostByNameAndDest(unsigned char *host, int qtype,
    ...
    if (sendQueryViaUDP(buf, query_len, dest) < 0) {
    ...
    for (i = 0; i < ntohs(header->ancount); i++) {
        if (ntohs(answer[i].resource->type) == qtype) {
            ...
            // Query this cname again to the same destination to get the
            // required address
        ...
        if (auth[i].rdata) {
            for (j = 0; j < ntohs(header->arcount); j++) {
                if (ntohs(addit[j].resource->type) == A_MASK &&
                    ...
                    if (inet_ntop(AF_INET, addit[j].rdata, ipv4,
                                  INET_ADDRSTRLEN) == NULL) {
                        perror("Address conversion failed");
                        continue;
                    }
...
void unpackDNSResponse(unsigned char *buf, dns_resource_record_t *answer,
    ...
    *header = (dns_header_t *)buf;
    qname =
        (unsigned char *)(buf +
                          sizeof(
                              dns_header_t)); // Point to right after the header
    ...
    for (i = 0; i < ntohs((*header)->ancount); i++) {
        ...
        if (ntohs(answer[i].resource->type) == A_MASK ||
            ntohs(answer[i].resource->type) == AAAA_MASK) {
            answer[i].rdata =
                (unsigned char *)malloc(ntohs(answer[i].resource->rdlength));
    ...
    for (i = 0; i < ntohs((*header)->nscount); i++) {
        ...
        if (ntohs(auth[i].resource->type) == A_MASK ||
            ntohs(auth[i].resource->type) == AAAA_MASK) {
            auth[i].rdata =
                (unsigned char *)malloc(ntohs(auth[i].resource->rdlength));
    ...
    for (i = 0; i < ntohs((*header)->arcount); i++) {
        ...
        if (ntohs(addit[i].resource->type) == A_MASK ||
            ntohs(addit[i].resource->type) == AAAA_MASK) {
            addit[i].rdata =
                (unsigned char *)malloc(ntohs(addit[i].resource->rdlength));
...
void packDNSQuery(unsigned char *host, int qtype, unsigned char *buf,
    ...
    *query_len += sizeof(dns_header_t);
    ...
    // qname = (unsigned char *) &buf[*query_len];	// Point to right after the
    // header
